...
const jsonFileName = process.env.FILE_NAME ? process.env.FILE_NAME.trim() : "days.json";

// Load schedule from days.json
let schedule = [];
try {
	const daysPath = path.join(__dirname, "data", jsonFileName);
	console.log(`Loading schedule from: ${daysPath}`);
...
if (!process.env.GESAC_LOGIN) {
 	console.error("Missing GESAC_LOGIN in .env file or on command line");
  	process.exit(1);
} else {
	loginAddress = process.env.GESAC_LOGIN.trim();
}
if (!process.env.GESAC_PASSWORD) {
 	console.error("Missing GESAC_LOGIN in .env file or on command line");
  	process.exit(1);
} else {
	loginPassword = process.env.GESAC_PASSWORD.trim();
}
...
let smtpEmailAddress = null;
let emailPassword = null;

if (!process.env.SMTP_EMAIL_ADDRESS) {
	console.error("⚠️  MISSING CREDENTIALS: Ensure SMTP_EMAIL_ADDRESS is set");
	process.exit(1);
} else {
	smtpEmailAddress = process.env.SMTP_EMAIL_ADDRESS.trim();

}

if (!process.env.SMTP_EMAIL_PASSWORD) {
	console.error("⚠️  MISSING CREDENTIALS: Ensure SMTP_EMAIL_PASSWORD is set");
	process.exit(1);
} else {
	emailPassword = process.env.SMTP_EMAIL_PASSWORD.trim();
...
let defaultUserClubId = null; // Store the club ID for booking verification
...
async function login() {
	...
	defaultUserClubId = jsonResponse.User.Member.DefaultClubId;
		...
		for (const entry of schedule) {
			...
			if (entry.checkday.toLowerCase() === currentDay) {
				...
				if (nowAEDT > dateBefore && nowAEDT < dateAfter) {
					...
					let userClubid = entry.clubId || defaultUserClubId; // Use entry club ID if available, otherwise use default from login response
					...
					if (classBookingStatus === "FullBooked"){
						console.log(`\t\tClass is full: ${entry.name} on ${entry.weekday} at ${entry.startTime}`);
						console.log('\t\tMax booking attempts reached. Moving to next entry.');
                                mailOptions = {
									...
									subject: 'Class NOT Booked - ' + classBookingStatus + ' - ' + eventName + ' - ' + entry.weekday + ' ' + entry.startTime,
									text: 'Class NOT Booked - ' + classBookingStatus + ' - ' + eventName + ' - ' + entry.weekday + ' ' + entry.startTime
								};
								
								try {
									const info = await transporter.sendMail(mailOptions);
									console.log('\t\tEmail sent successfully to:', loginAddress);
									console.log('\t\tClass NOT Booked - ' + classBookingStatus + ' - ' + eventName + ' - ' + entry.weekday + ' ' + entry.startTime);
                    ...
                    // this is to avoid unnecessary booking attempts and potential issues with the booking system. 
					if (classBookingStatus === "Bookable" || classBookingStatus === "Awaitable") {
						...
						while (!bookingStatus) { // while we havent yet books, we will keep trying every minute
                            ...
                            // pause for a 5 seconds before trying again to avoid spamming the booking endpoint and to give it time to update the booking status, 
                            // if we keep trying too fast it may cause issues with the booking system or get us rate limited.
                            await new Promise(r => setTimeout(r, 5000));
                            attempts++;
                            // if we have attempted to book 72 times (about 12 minutes) and still haven't succeeded, 
